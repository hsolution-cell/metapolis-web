...
import { SUB_PAGES, getPageMeta } from "@/data/siteMeta";
...
const GROUPS: { title: string; links: PreviewLink[] }[] = [
  ...
  {
    title: "매장안내",
    links: SUB_PAGES.slice(4, 8).map((p) => ({ href: p.path, label: p.label, legacy: `${p.legacy}.html` })),
  },
  {
    title: "이벤트",
    links: SUB_PAGES.slice(8, 11).map((p) => ({ href: p.path, label: p.label, legacy: `${p.legacy}.html` })),
  },
  {
    title: "고객센터",
    links: SUB_PAGES.slice(11, 14).map((p) => ({ href: p.path, label: p.label, legacy: `${p.legacy}.html` })),
  },
];
