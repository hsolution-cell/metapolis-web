...
- 국문 사이트를 그대로 번역하는 방식이 **아님** — 구조·메뉴가 완전히 다른 별도 사이트
- 접속 경로: **`/en`** (`app/en/*` 서브트리로 구성)
- [ ] `/en` 라우트 구조 신규 구성 (국문 컴포넌트 재사용 여부 검토, 별도 레이아웃/헤더 가능성 높음)
- [ ] EN 전용 메뉴 5개만 구성:
  - [ ] 메인 페이지
  - [ ] 메타폴리스 소개
  - [ ] 영업시간
  - [ ] 주소 및 주차안내
  - [ ] 층별안내
- [ ] `components/layout/Header.tsx:136` — ENG 클릭 시 "준비 중" 토스트 → `/en` 이동으로 교체
- [ ] EN 콘텐츠(번역문·이미지) 데이터 확보
- [ ] `sitemap.ts` / `robots.ts` / `hreflang`에 `/en` 반영
- [ ] **별도 견적·일정 필요.**
